...
### 3. ポケカ買取チェッカーの価格取得（ポケモン追加参照）

pokeca-box-hikaku.com の全8店舗最高値を基準にポケモン価格を更新する。
fetch-pokeca.yml には push トリガーが設定済みのため、トリガーファイルをプッシュするだけでよい:

1. **トリガーファイルをプッシュ**（`mcp__github__push_files` で `trigger-pokeca` ファイルを作成）
2. **完了を待つ**（約1〜2分。`mcp__github__list_commits` で `github-actions[bot]` のコミットを確認）
3. **後片付け**: `mcp__github__delete_file` で `trigger-pokeca` を削除
4. **ローカルにフェッチ**: `git fetch origin <branch> && git checkout origin/<branch> -- data/pokeca_MM_DD_pokemon.json`

取得後、以下のコマンドでスプレッドシートを更新する（マージン: +200円）:

```bash
python main.py --from-json data/pokeca_MM_DD_pokemon.json -g pokemon --margin-box 200 --yes
```

※ price_2（カートン）はサイト非対応のため null。ポケモンのシュリンクあり価格のみ更新される。
※ ほむらとの比較後に実施し、チェッカー最高値+マージンがほむらを上回る場合のみ更新する。
...
### ポケカ買取チェッカー基準（ポケモン追加参照）

| ゲーム | BOX（シュリンクあり） | カートン（シュリンクなし） |
|--------|----------------------|--------------------------|
| ポケモン | +200円 | 非対応（null） |

**参照元**: pokeca-box-hikaku.com（全8店舗: 森森買取・ほむら東京・一丁目買取・ラント・カードソムリエ・買強・昇天・ルーデヤ）
**更新頻度**: サイト側で1日3回更新。`data/pokeca_M_D_pokemon.json` として保存。
